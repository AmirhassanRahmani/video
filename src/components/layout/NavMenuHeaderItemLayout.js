...
const NavMenuHeaderItemLayout = ({ menu, index, headerBg }) => {
    ...
    <>
      ...
      >
        <span
          className={`${
            index === 0 &&
            'relative before:content-[""] before:w-[.5rem] before:h-[.5rem] before:bg-red-500 before:absolute before:rounded-full before:top-[.6rem] before:left-[-.9rem] max-[1130px]:hidden'
          } text-[.95rem] font-medium text-gray-500 transition ease delay-50 hover:text-gray-600 ${
            index === 6 && `relative `
          }`}
        >
          {menu.nav}
...
export default NavMenuHeaderItemLayout;
