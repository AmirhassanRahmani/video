...
const PageThree = () => {
  ...
  const [cardThree, setCardThree] = useState(
    listCardVideosThree.find((list) => list.url === params.vidioId)
  );
        ...
        {cardThree &&
          cardThree.data.map((item, index) => (
            <div>
              ...
              <div
                className={`scrollNameFilmAndButtons fixed top-[3.3rem] w-full ${
                  goUp ? "grid" : "hidden"
        ...
        <div className="flex justify-between w-[80%] mx-auto max-[1030px]:hidden">
          ...
          >
            {listAsideEpisode.map((item, index) => (
              <>
                {cardThree.hasOwnProperty(item.id) && (
                  <a
                    href={item.href}
              ...
              <div className="kontenAndSeason flex items-center justify-between mt-8">
                {cardThree.konten && (
                  <div className="konten font-bold text-[1.3rem]" id="konten">
                    Serupa dengan {cardThree.data[0].filmName}
                  </div>
                )}
              </div>
